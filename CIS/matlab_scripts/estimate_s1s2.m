function [S1_locations_final, S2_locations_final] = estimate_s1s2(data_path, fileName)
...
[A0]=denoising(data_path, fileName);
sig = A0;
...
% 	    S2 = stem([i:i+wlen-1]/FS,S2_detect(i:i+wlen-1)>0,'b');
        S1 = stem((find(S1_detect(i:i+wlen-1)>0)+i-1)/FS,...
            S1_detect(find(S1_detect(i:i+wlen-1)>0)+i-1),'r');
        S2 = stem((find(S2_detect(i:i+wlen-1)>0)+i-1)/FS,...
            S2_detect(find(S2_detect(i:i+wlen-1)>0)+i-1),'b');
        hold off
        legend([S1, S2], {'S1','S2'}, 'Location','northeast', 'NumColumns',1);
        legend boxoff;
        %pause
%         baseFileName = sprintf('%s_%s_frame%02d.jpg',string(fileName), i);
...
%         frame=getframe(gcf); 
%         imwrite(frame.cdata, localFileName);
% 
