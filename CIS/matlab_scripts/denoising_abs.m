...
med75 =prctile(abs(D4), 50); % Level of noise present 
m = mean(abs(D4));
v = var(D4);
% syms T;
...
M=median(abs(D4)); % median of absolute value of detail coefficients at level 4
...
% condition-wise estimation of threshold of D4
med75 =prctile(abs(D5), 50); % Level of noise present 
m = mean(abs(D5));
v = var(D5);
% syms T;
% T = piecewise(med75 < v, med75*[1-(v-med75)] , med75 > v & med75 < m, med75, med75 > m, med75+(med75-m));
...
M=median(abs(D5)); % median of absolute value of detail coefficients at level 4
noiseVariance=M/0.6745; 
T_rescaled=T*noiseVariance; %rescale of estimated threshold value based on mln  
